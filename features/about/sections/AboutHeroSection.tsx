...
import { Reveal, Stagger, NumberTicker } from "@/components/animation";
...
const AboutHeroSection = async ({
  ...
  const items = [
    {
      value: gymCount ? <NumberTicker value={gymCount} /> : "—",
      label: t.gymsLabel,
    },
    {
      value: packageCount ? <NumberTicker value={packageCount} /> : "—",
      label: t.packagesLabel,
    },
    { value: <NumberTicker value={3} />, label: t.languagesLabel },
    { value: <NumberTicker value={11} />, label: t.teamLabel },
  ];
  ...
  return (
    <section className="relative overflow-hidden bg-surface">
      ...
      <Container className="relative flex flex-col gap-10 py-16 md:py-20">
        ...
        <Stagger variant="rise" className="flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
          {items.map((stat) => (
